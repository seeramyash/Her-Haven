...
def get_personalized_recommendations(df):
    ...
    <div style="background: linear-gradient(135deg, #ffb3b3 0%, #ffc7a3 50%, #ffd9b3 100%); padding: 20px; border-radius: 15px; color: #2d1b1b; margin-bottom: 20px;">
    ...
    <div style="margin: 20px 0;">
        <h3 style="color: #d4a5a5;">📊 Current Status</h3>
    ...
    recommendations_html += '<h3 style="color: #d4a5a5;">🎯 Priority Recommendations</h3>'
    ...
    for priority in priorities[:3]:  # Show top 3 priorities
        recommendations_html += f"""
        <div style="background-color: {priority['color']}44; border-left: 5px solid {priority['color']}; padding: 15px; margin: 15px 0; border-radius: 5px;">
            <h4 style="color: #d4a5a5; margin-top: 0;">{priority['title']}</h4>
    ...
    recommendations_html += '<h3 style="color: #d4a5a5;">🍽️ Nutrition Recommendations</h3>'
    ...
    if len(period_entries) > 0:
        ...
        if any(symptoms.values()):
            recommendations_html += '<h3 style="color: #d4a5a5;">🩺 Symptom Management</h3>'
            
            symptom_recommendations = get_symptom_recommendations(symptoms)
            for symptom, advice in symptom_recommendations.items():
                recommendations_html += f"""
                <div style="background-color: rgba(255, 227, 217, 0.4); padding: 12px; margin: 10px 0; border-radius: 8px;">
                    <strong>{symptom}:</strong> {advice}
                </div>
                """
    
    # Wellness activities
    recommendations_html += '<h3 style="color: #d4a5a5;">🌟 Recommended Wellness Activities</h3>'
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin: 20px 0;">
        <div>
            <h4 style="color: #d4a5a5;">🧘 Mind & Spirit</h4>
    ...
    recommendations_html += """
        ...
        <div>
            <h4 style="color: #d4a5a5;">💪 Body & Movement</h4>
            <ul style="line-height: 1.8;">
    """
    ...
    if avg_wellness >= 70:
        recommendations_html += """
        <div style="background: linear-gradient(135deg, #ffb3b3 0%, #ffc7a3 100%); color: #2d1b1b; padding: 20px; border-radius: 10px; margin: 20px 0; text-align: center;">
            <h3>🎉 Great job!</h3>
        ...
        recommendations_html += """
        <div style="background: linear-gradient(135deg, #ffc7a3 0%, #ffd9b3 100%); color: #2d1b1b; padding: 20px; border-radius: 10px; margin: 20px 0; text-align: center;">
        ...
        recommendations_html += """
        <div style="background: linear-gradient(135deg, #ffd9b3 0%, #ffe5d9 100%); color: #2d1b1b; padding: 20px; border-radius: 10px; margin: 20px 0; text-align: center;">
...
def get_cycle_nutrition_recommendations(phase, recent_data):
    ...
    html = f"""
    <div style="background: linear-gradient(135deg, #ffb3b3 0%, #ffc7a3 100%); padding: 20px; border-radius: 10px; color: #2d1b1b; margin: 15px 0;">
        <h4>🌙 {phase} Phase Nutrition</h4>
        <p><strong>Focus:</strong> {phase_rec['focus']}</p>
    </div>
    
    <div style="background-color: #fff5f0; padding: 15px; border-radius: 10px; margin: 10px 0;">
    ...
    <div style="background-color: #ffe5d9; padding: 12px; border-radius: 8px; margin: 10px 0;">
