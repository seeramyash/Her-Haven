...
def generate_weekly_report(df):
    ...
    # Build HTML
    html = f"""
    <div style="background: linear-gradient(135deg, #ffb3b3 0%, #ffc7a3 50%, #ffd9b3 100%); padding: 25px; border-radius: 15px; color: #2d1b1b; margin-bottom: 25px;">
    ...
    # Metric cards
    wellness_color = "#ffb3b3" if avg_wellness >= 70 else "#ffc7a3" if avg_wellness >= 50 else "#ff9999"
    html += f"""
        <div style="background-color: {wellness_color}44; padding: 15px; border-radius: 10px; text-align: center;">
    ...
    sleep_color = "#ffd9b3" if avg_sleep >= 7 else "#ffc7a3"
    html += f"""
        <div style="background-color: {sleep_color}44; padding: 15px; border-radius: 10px; text-align: center;">
            <h3 style="color: {sleep_color}; margin: 0;">{avg_sleep:.1f}h</h3>
            <p style="margin: 5px 0;">Avg Sleep</p>
        </div>
    """
    
    exercise_color = "#ffb3b3" if total_exercise >= 150 else "#ffc7a3"
    html += f"""
        <div style="background-color: {exercise_color}44; padding: 15px; border-radius: 10px; text-align: center;">
            <h3 style="color: {exercise_color}; margin: 0;">{total_exercise:.0f}min</h3>
            <p style="margin: 5px 0;">Total Exercise</p>
        </div>
    """
    
    stress_color = "#ff9999" if avg_stress > 6 else "#ffc7a3" if avg_stress > 4 else "#ffb3b3"
    html += f"""
        <div style="background-color: {stress_color}44; padding: 15px; border-radius: 10px; text-align: center;">
    ...
    # Key insights
    html += '<h3 style="color: #d4a5a5;">🔍 Key Insights</h3>'
    ...
    for insight in insights:
        if insight['type'] == 'success':
            html += f"""
            <div style="background-color: rgba(255, 227, 217, 0.4); border-left: 5px solid #ffb3b3; padding: 15px; margin: 10px 0; border-radius: 5px;">
                <strong style="color: #d4a5a5;">{insight['icon']} {insight['title']}</strong>: {insight['message']}
            </div>
            """
        elif insight['type'] == 'warning':
            html += f"""
            <div style="background-color: rgba(255, 199, 174, 0.4); border-left: 5px solid #ffc7a3; padding: 15px; margin: 10px 0; border-radius: 5px;">
                <strong style="color: #d4a5a5;">{insight['icon']} {insight['title']}</strong>: {insight['message']}
            </div>
            """
        else:
            html += f"""
            <div style="background-color: rgba(255, 217, 179, 0.4); border-left: 5px solid #ffd9b3; padding: 15px; margin: 10px 0; border-radius: 5px;">
                <strong style="color: #d4a5a5;">{insight['icon']} {insight['title']}</strong>: {insight['message']}
            </div>
            """
    
    # Daily breakdown table
    html += '<h3 style="color: #d4a5a5; margin-top: 30px;">📊 Daily Breakdown</h3>'
    html += '<div style="overflow-x: auto;"><table style="width: 100%; border-collapse: collapse;">'
    html += '<tr style="background: linear-gradient(135deg, #ffb3b3 0%, #ffc7a3 100%); color: #2d1b1b;"><th style="padding: 10px;">Date</th><th>Wellness</th><th>Sleep</th><th>Exercise</th><th>Stress</th><th>Water</th></tr>'
    ...
    # Action items
    html += '<h3 style="color: #d4a5a5; margin-top: 30px;">🎯 Action Items for Next Week</h3>'
    ...
    if action_items:
        ...
        html += """
        <div style="background: linear-gradient(135deg, #ffb3b3 0%, #ffc7a3 100%); color: #2d1b1b; padding: 20px; border-radius: 10px; text-align: center; margin: 20px 0;">
...
def generate_monthly_report(df, ml_predictor):
    ...
    if len(recent_data) < 7:
        return "<div style='background-color: rgba(255, 217, 179, 0.4); border-left: 5px solid #ffd9b3; padding: 15px; margin: 10px 0; border-radius: 5px;'><strong>ℹ️ Need at least 7 days of data for a comprehensive monthly report.</strong></div>"
    ...
    # Monthly Overview
    html += '<h3 style="color: #d4a5a5;">📊 Monthly Overview</h3>'
    html += '<div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; margin-bottom: 25px;">'
    
    period_days = int(recent_data['on_period'].sum()) if 'on_period' in recent_data else 0
    
    html += f"""
        <div style="background-color: rgba(255, 227, 217, 0.3); padding: 15px; border-radius: 10px; text-align: center;">
            <h3 style="color: #d4a5a5; margin: 0;">{len(recent_data)}</h3>
            <p style="margin: 5px 0;">Total Entries</p>
        </div>
        
        <div style="background-color: rgba(255, 227, 217, 0.3); padding: 15px; border-radius: 10px; text-align: center;">
            <h3 style="color: #d4a5a5; margin: 0;">{(recent_data['wellness_score'].mean() if 'wellness_score' in recent_data else 0):.1f}</h3>
            <p style="margin: 5px 0;">Avg Wellness Score</p>
        </div>
        
        <div style="background-color: rgba(255, 227, 217, 0.3); padding: 15px; border-radius: 10px; text-align: center;">
            <h3 style="color: #d4a5a5; margin: 0;">{recent_data['exercise_minutes'].sum():.0f}min</h3>
            <p style="margin: 5px 0;">Total Exercise</p>
        </div>
    """
    ...
    html += f"""
        <div style="background-color: rgba(255, 227, 217, 0.3); padding: 15px; border-radius: 10px; text-align: center;">
            <h3 style="color: #d4a5a5; margin: 0;">{recent_data['sleep_hours'].mean():.1f}h</h3>
            <p style="margin: 5px 0;">Avg Sleep</p>
        </div>
        
        <div style="background-color: rgba(255, 227, 217, 0.3); padding: 15px; border-radius: 10px; text-align: center;">
            <h3 style="color: #d4a5a5; margin: 0;">{period_days}</h3>
            <p style="margin: 5px 0;">Period Days</p>
        </div>
        
        <div style="background-color: rgba(255, 227, 217, 0.3); padding: 15px; border-radius: 10px; text-align: center;">
            <h3 style="color: #d4a5a5; margin: 0;">{recent_data['average_stress'].mean():.1f}/10</h3>
            <p style="margin: 5px 0;">Avg Stress</p>
        </div>
    """
    ...
    html += '<h3 style="color: #d4a5a5; margin-top: 30px;">🎯 Monthly Achievements</h3>'
    ...
    if achievements:
        html += '<div style="background-color: rgba(255, 227, 217, 0.4); padding: 15px; border-radius: 10px; margin: 10px 0;">'
        for achievement in achievements:
            html += f'<div style="margin: 8px 0;">✅ {achievement}</div>'
        html += '</div>'
    else:
        html += '<div style="background-color: rgba(255, 217, 179, 0.4); border-left: 5px solid #ffd9b3; padding: 15px; margin: 10px 0; border-radius: 5px;"><strong>ℹ️ Keep tracking to unlock achievements!</strong></div>'
    
    # Goals for next month
    html += '<h3 style="color: #d4a5a5; margin-top: 30px;">🚀 Goals for Next Month</h3>'
