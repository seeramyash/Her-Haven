...
const CycleForecast = () => {
  ...
  const getConfidenceColor = (confidence) => {
    if (confidence === 'High') return '#ffb3b3';
    if (confidence === 'Medium') return '#ffc7a3';
    return '#ff9999';
  };

  const getCategoryColor = (category) => {
    if (category === 'Very Likely') return '#ffb3b3';
    if (category === 'Likely') return '#ffc7a3';
    if (category === 'Possible') return '#ffd9b3';
    return '#e8d5c4';
  };
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: '20px' }}>
        <div style={{
          background: 'linear-gradient(135deg, #ffb3b3 0%, #ffc7a3 50%, #ffd9b3 100%)',
          padding: '20px',
          borderRadius: '10px',
          color: '#2d1b1b',
          marginBottom: '20px'
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px' }}>
          <div style={{
            backgroundColor: 'rgba(255, 227, 217, 0.3)',
            padding: '15px',
            borderRadius: '8px',
            textAlign: 'center'
          }}>
            <p style={{ margin: 0, fontSize: '0.9em', color: '#666' }}>Date</p>
            <h3 style={{ margin: '5px 0', color: '#d4a5a5' }}>
          ...
          <div style={{
            ...
            <h3 style={{ margin: '5px 0', color: '#d4a5a5' }}>{prediction.days_until}</h3>
          </div>
      ...
      {symptoms && (
        <div className="card">
          <h2 style={{ color: '#d4a5a5', marginBottom: '20px' }}>🩺 Symptom Predictions</h2>
            ...
            {Object.entries(symptoms).map(([symptom, data]) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: '10px',
                  padding: '15px',
                  backgroundColor: getCategoryColor(data.category) === '#ffb3b3' 
                    ? 'rgba(255, 179, 178, 0.3)' 
                    : getCategoryColor(data.category) === '#ffc7a3' 
                    ? 'rgba(255, 199, 163, 0.3)' 
                    : getCategoryColor(data.category) === '#ffd9b3' 
                    ? 'rgba(255, 217, 179, 0.3)' 
                    : 'rgba(232, 213, 196, 0.3)'
                }}
              >
